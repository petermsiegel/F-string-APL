...
| \`W                                      | Wrap                                 | `` [L R \| LR] `W ⍵` ``. Wraps `⍵`in decorators`L` (`0⊃⍺`) and `R` (`1⊃⍺`). `⍵`is an object of any shape and depth.`L`and `R`are char. vectors or scalars or`⍬`(same as`''`); if there is one scalar or enclosed vector, it is replicated (`L R←2⍴⍺`). Each row (or vector) of `⍵`will have`L`catenated on the left and`R`on the right. By default,`⍺←''''`, i.e. APL quotes will wrap the object ⍵, row by row, whether character, numeric or otherwise. **EXPERIMENTAL** |
